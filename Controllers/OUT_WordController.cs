...
{
    public class OUT_WordController : Controller
    {
        private readonly AppDbContext _appDbContext;
        private readonly JwtTokenHelper _jwtTokenHelper;
        private readonly ILogger<OUT_WordController> _logger;

        public OUT_WordController(AppDbContext appDbContext, JwtTokenHelper jwtTokenHelper, ILogger<OUT_WordController> logger)
        {
        ...
        public int[] GetMucDanhGiaById(int id)
        {
            ...
            var mucDanhGia = _appDbContext.OUT_MauKhaoSat
                                     .Where(m => m.IdOUT_MauKhaoSat == id)
                                     .Select(m => m.MucDanhGia) // Giả sử MucDanhGia là kiểu int[]
                                     .FirstOrDefault();
            return mucDanhGia; // Trả về mảng MucDanhGia

        }
        public List<OUT_NhomCauHoiKhaoSat> GetNhomCauHoiByTitles(string[] nhomCauHoi)
        {
            // Lấy ra các nhóm câu hỏi tương ứng với tiêu đề
            var nhomCauHoiList = _appDbContext.OUT_NhomCauHoiKhaoSat
                .Where(n => nhomCauHoi.Contains(n.TieuDe))
                .ToList();
            return nhomCauHoiList;
        }
        public List<OUT_CauHoiKhaoSat> GetCauHoiByTitles(string[] cauHoi)
        {
            // Lấy ra các nhóm câu hỏi tương ứng với tiêu đề
            var cauHoiList = _appDbContext.OUT_CauHoiKhaoSat
        ...
        public IActionResult ExportToWord(int id)
        {
            ...
            var OUT_MauKhaoSat = _appDbContext.OUT_MauKhaoSat.Find(id);
            if (OUT_MauKhaoSat != null)
            {
                string[] nhomCauHoi = OUT_MauKhaoSat?.NhomCauHoiKhaoSat ?? Array.Empty<string>();
                string[] cauHoi = OUT_MauKhaoSat?.CauHoiKhaoSat ?? Array.Empty<string>();
                int[] mucDanhGia = OUT_MauKhaoSat?.MucDanhGia ?? Array.Empty<int>();
                List<OUT_NhomCauHoiKhaoSat> nhomCauHois = GetNhomCauHoiByTitles(nhomCauHoi);
                List<OUT_CauHoiKhaoSat> cauHois = GetCauHoiByTitles(cauHoi);
            ...
            using (var ms = new MemoryStream())
            {
                ...
                using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(ms, DocumentFormat.OpenXml.WordprocessingDocumentType.Document, true))
                {
                    ...
                    // Tạo bảng câu hỏi
                    CreateQuestionTable(body, OUT_MauKhaoSat);
                    body.Append(new Paragraph(new Run(new Text("")))); // Dòng trống
        ...
        private void CreateSurveyDocument(Body body)
        {
            ...
            headerCell3.Append(CreateFormattedParagraph("PHIẾU KHẢO SÁT Ý KIẾN NGƯỜI BỆNH NGOẠI TRÚ"));
        ...
        private void AddPatientInfoSection(Body body)
        {
            ...
            runQuestionA4.Append(new Text("A4. Ước tính khoảng cách từ nơi sinh sống đến bệnh viện:................. km"));
            ...
            runQuestionA5.Append(new Text("A5. Ông/Bà có sử dụng thẻ BHYT cho lần khám bệnh này không?\t"));
        ...
        private void CreateQuestionTable(Body body, OUT_MauKhaoSat OUT_MauKhaoSat)
        {
            string[] nhomCauHoi = OUT_MauKhaoSat.NhomCauHoiKhaoSat ?? Array.Empty<string>();
            string[] cauHoi = OUT_MauKhaoSat.CauHoiKhaoSat ?? Array.Empty<string>();
            int[] mucDanhGia = OUT_MauKhaoSat.MucDanhGia ?? Array.Empty<int>(); // Lấy mức đánh giá cho mẫu khảo sát
            ...
            foreach (var nhom in nhomCauHois)
            {
                ...
                var filteredCauHois = cauHois
                    .Where(c => c.IdOUT_NhomCauHoiKhaoSat == nhom.IdOUT_NhomCauHoiKhaoSat)
                    .ToList();
        ...
        private void CreateFinalEvaluationTable(Body body)
        {
            ...
            TableCell cell1_1 = new TableCell(new Paragraph(new Run(new Text("F"))));
            ...
            TableCell cell2_1 = new TableCell(new Paragraph(new Run(new Text("G"))));
            ...
            Run run3 = new Run(new Text("3.Có thể sẽ quay lại"));
