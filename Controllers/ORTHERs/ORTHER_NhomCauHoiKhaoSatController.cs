...
{
    public class ORTHER_NhomCauHoiKhaoSatController : Controller
    {
        private readonly AppDbContext _context;
        private readonly JwtTokenHelper _jwtTokenHelper;
        private readonly ILogger<ORTHER_NhomCauHoiKhaoSatController> _logger;

        public ORTHER_NhomCauHoiKhaoSatController(AppDbContext context, JwtTokenHelper jwtTokenHelper, ILogger<ORTHER_NhomCauHoiKhaoSatController> logger)
        {
            _context = context;
        ...
        public async Task<IActionResult> Show_ORTHER_NhomCauHoiKhaoSat()
        {
            try
            {
                ...
                var nhomCauHoiKhaoSats = await _context.ORTHER_NhomCauHoiKhaoSat
                    ...
                    .ToListAsync();
                // Log information  
                _logger.LogInformation("Successfully retrieved ORTHER_NhomCauHoiKhaoSat data.");
                return PartialView("_Show_ORTHER_NhomCauHoiKhaoSat", nhomCauHoiKhaoSats);
            }
            catch (Exception ex)
            {
                // Log the error
        ...
        public async Task<IActionResult> ThemNhomCauHoiKhaoSat([FromBody] TitleAndContentList data)
        {
            ...
            try
            {
                ...
                {
                    var newGroup = new ORTHER_NhomCauHoiKhaoSat
                    {
                    ...
                    var existingRecord = await _context.ORTHER_NhomCauHoiKhaoSat
                    ...
                    _context.ORTHER_NhomCauHoiKhaoSat.Add(newGroup);
                }
        ...
        public async Task<IActionResult> XoaNhomCauHoiKhaoSat([FromBody] DeleteRequest request)
        {
            try
            {
                var nhom = await _context.ORTHER_NhomCauHoiKhaoSat.FindAsync(request.Id);
                if (nhom == null)
                {
                    return Json(new { success = false, message = "Nhóm câu hỏi không tồn tại." });
                }
                bool isUsedInInSurvey = await _context.ORTHER_MauKhaoSat.AnyAsync(m => m.NhomCauHoiKhaoSat.Contains(nhom.TieuDe));
                if (isUsedInInSurvey)
                {
                    return Json(new { success = false, message = "Nhóm câu hỏi này đang được sử dụng trong mẫu khảo sát." });
                }

                _context.ORTHER_NhomCauHoiKhaoSat.Remove(nhom);
                await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> CapNhatNhomCauHoiKhaoSat([FromBody] List<UpdateRequest> request)
        {
            ...
            try
            {
                foreach (var item in request)
                {
                    ...
                    var existingItem = await _context.ORTHER_NhomCauHoiKhaoSat.FindAsync(item.Id);
