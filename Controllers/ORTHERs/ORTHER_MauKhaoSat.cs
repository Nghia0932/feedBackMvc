...
{
    public class ORTHER_MauKhaoSatController : Controller
    {
        private readonly AppDbContext _context;
        private readonly JwtTokenHelper _jwtTokenHelper;
        private readonly ILogger<ORTHER_MauKhaoSatController> _logger;

        public ORTHER_MauKhaoSatController(AppDbContext context, JwtTokenHelper jwtTokenHelper, ILogger<ORTHER_MauKhaoSatController> logger)
        {
            _context = context;
        ...
        public async Task<IActionResult> Show_ORTHER_MauKhaoSat()
        {
            try
            {
                var nhomCauHoiKhaoSats = await _context.ORTHER_NhomCauHoiKhaoSat
                    .Include(n => n.CauHoiKhaoSats) // Include related entities
                    .ToListAsync();
                return PartialView("_Show_ORTHER_MauKhaoSat", nhomCauHoiKhaoSats);
            }
            catch (Exception ex)
            {
                // Log the error
                _logger.LogError(ex, "An error occurred while retrieving ORTHER_NhomCauHoiKhaoSat data.");
        ...
        public class ThemMauKhaoSat_Request
        {
            ...
            public List<int>? MucDanhGia { get; set; }
        }
        [HttpPost]
        public async Task<IActionResult> ThemMauKhaoSat([FromBody] ThemMauKhaoSat_Request request)
        {
            ...
            var existingRecord = await _context.ORTHER_MauKhaoSat
            ...
            var mauKhaoSat = new ORTHER_MauKhaoSat
            {
                ...
                MucDanhGia = request.MucDanhGia?.ToArray()
            };
            try
            {
                _context.ORTHER_MauKhaoSat.Add(mauKhaoSat);
        ...
        [HttpPost]
        public async Task<IActionResult> Xoa_ORTHER_MauKhaoSat([FromBody] DeleteRequest request)
        {
            try
            {
                var mauKhaoSat = await _context.ORTHER_MauKhaoSat.FindAsync(request.Id);
                ...
                _context.ORTHER_MauKhaoSat.Remove(mauKhaoSat);
                await _context.SaveChangesAsync();
        ...
        [HttpPost]
        public async Task<IActionResult> CapNhat_ORTHER_MauKhaoSat([FromBody] UpdateRequest request)
        {
            if (request == null)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> CapNhatTrangThai_ORTHER_MauKhaoSat([FromBody] UpdateStatusRequest request)
        {
            if (request == null)
            {
                return BadRequest(new { success = false, message = "Không nhận được dữ liệu cập nhật." });
            ...
            try
            {
                if (request.isOpen != null)
                {
                    ...
                    var allEntries = _context.ORTHER_MauKhaoSat.ToList();
